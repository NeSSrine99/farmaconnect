...
export default function ProductCard({
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.03 }}
      transition={{ type: "spring", stiffness: 300 }}
      className=" bg-white rounded-xl shadow-md hover:shadow-lg transition duration-300 overflow-hidden flex flex-row relative w-full p-2 border"
    >
      {/* Image */}
      <div className="flex items-center gap-2">
        <img
          src={image}
          alt={name}
          className="max-h-full max-w-full object-contain rounded-full shadow-2xs "
          width={60}
          height={60}
        />
        <div className="flex flex-col gap-1">
          <p className="text-textLight text-xs">{brand} </p>
          <h3 className=" font-medium text-textDark text-sm ">{name} </h3>
        </div>
      </div>
    </motion.div>
  );
